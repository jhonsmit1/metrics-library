...
let AuthMetricsInterceptor = class AuthMetricsInterceptor {
    metricsService;
    ...
    intercept(context, next) {
        ...
        return next.handle().pipe((0, operators_1.catchError)((error) => {
            if (error instanceof common_1.UnauthorizedException ||
                error?.status === 401 ||
                error?.status === 403) {
                this.metricsService.recordAuthFailure("invalid_token", endpoint);
            }
...
exports.AuthMetricsInterceptor = AuthMetricsInterceptor = __decorate([
